/*
 * cordova.oauth2.js - v0.1.1-2
 * 2nd edition revised for working normally.
...
(function($) {
    $.oauth2 = function(options, successCallback, errorCallback) {
        ...
        var checkOauth2Params = function(options) {
            var missing_params = "";
            if (!options.client_id) missing_params += " client_id";
            if (!options.auth_url) missing_params += " auth_url";
            if (!options.response_type) missing_params += " response_type";
            if (!options.client_secret && options.response_type == "code") missing_params += " client_secret";
            if (!options.token_url && options.response_type == "code") missing_params += " token_url";
            if (!options.redirect_uri) missing_params += " redirect_uri";
            if (missing_params) {
                var error_msg = "Missing OAuth2 parameters:" + missing_params;
        ...
        // "Loadstart" is not applicable. Instead, "loadend" or "loaderror" should be used.
        // Because invalid redirect_uri will lead to an loaderror event.
        $(loginWindow).on('loaderror', function(e) {
            ...
            // if implicit method check for acces_token/error in url hash fragment
                ...
                var access_token_temp = url.split("access_token=")[1];
                var access_token = access_token_temp.split("&")[0];
